    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(249,115,22,0.14),_transparent_26%),linear-gradient(180deg,_#fffaf5_0%,_#ffffff_45%,_#f8fafc_100%)]">
      <header className="sticky top-0 z-20 border-b border-white/50 bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/auth/register"><Button className="rounded-full bg-slate-950 hover:bg-slate-800">Launch Listing</Button></Link>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <section className="rounded-[2rem] bg-slate-950 px-6 py-8 text-white shadow-[0_40px_120px_rgba(15,23,42,0.22)] lg:px-10">
          <p className="text-xs uppercase tracking-[0.35em] text-orange-300">Public Marketplace</p>
        ...
        <Card className="mt-6 rounded-[2rem] border-slate-200 bg-white/85 shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
          <CardContent className="p-6">
            <form method="GET" className="space-y-4">
              <div className="grid gap-4 lg:grid-cols-[1.4fr_0.8fr_0.8fr_auto]">
                ...
                <select name="category" defaultValue={searchParams.category} className="h-12 rounded-full border border-slate-200 bg-white px-4 text-sm">
                  <option value="">All categories</option>
                  {categories?.map((category: any) => (
                    <option key={category.id} value={category.slug}>{category.name}</option>
                  ))}
                </select>
                <select name="city" defaultValue={searchParams.city} className="h-12 rounded-full border border-slate-200 bg-white px-4 text-sm">
                  <option value="">All cities</option>
                  {cities?.map((city: any) => (
                    <option key={city.id} value={city.slug}>{city.name}</option>
                  ))}
                </select>
                <Button type="submit" className="h-12 rounded-full bg-slate-950 px-6 hover:bg-slate-800">
